  ...
  def build_pubdate
    if self.base_sheet.base_series_abbr=='wsk'
      ...
      if pubyear == 'ND' || pubyear.nil? || pubyear==''
        pubyear = [self.bewerkt.to_s.last(4).to_i, self.verkend.to_s.last(4).to_i, self.bijgewerkt.to_s.last(4).to_s.to_i, self.herzien.to_s.last(4).to_i, self.opname_jaar.to_s.last(4).to_i, self.basis_jaar.to_s.last(4).to_i].max.to_s
        exact = false
      end
